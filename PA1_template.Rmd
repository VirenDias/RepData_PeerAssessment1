...
The mean is ``r format(meanSteps, scientific = FALSE)`` and the median is ``r medianSteps``.
...
The 5-minute time interval corresponding to the maximum average number of steps is ``r maxInterval``.
...
The total number of missing values in the dataset is ``r sum(missingValues)``.
...
```{r meanMedianStepsImputed}
meanStepsImputed <- mean(totalStepsPerDayImputed$steps)
medianStepsImputed <- median(totalStepsPerDayImputed$steps)
```

The mean is ``r format(meanStepsImputed, scientific = FALSE)`` and the median is ``r format(medianStepsImputed, scientific = FALSE)``. 
